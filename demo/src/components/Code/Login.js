...
const Login = ({ setUserState }) => {
  ...
  const loginHandler = (e) => {
    ...
    setFormErrors(validateForm(user));
    setIsSubmit(true);
    // axios.post('https://localhost:44354/api/GetKey', user)
    // .then(res => {
    //     Cookies.set("JobSeekerID", res.data);
    // })
axios.post('https://localhost:44354/api/JobSeekerSignup', user)
    .then(res => {
        Cookies.set("JobSeekerID", res.data.jobSeekerID);
    })
axios.post('https://localhost:44354/api/Login', user)
    .then(res => {
